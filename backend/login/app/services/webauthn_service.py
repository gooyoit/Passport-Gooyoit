...
def begin_authentication(db: Session, client_id: str) -> tuple[dict, str]:
    """Generate a WebAuthn challenge for login."""
    ...
    credentials = db.scalars(select(WebAuthnCredentialModel)).all()
    if not credentials:
...
def verify_authentication(
    ...
    *,
    ...
    """Verify a WebAuthn authentication assertion and issue auth code."""
    application = get_active_application_by_client_id(db, client_id)
    if application is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Application not found")
...
def begin_registration(db: Session, client_id: str, email: str, display_name: str | None = None) -> tuple[dict, str]:
    ...
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found. Please register with email first.")
    ...
    options = generate_registration_options(
        ...
        authenticator_selection={"authenticatorAttachment": "cross-platform", "userVerification": "preferred"},
        timeout=60000,
        attestation="none",
    )
